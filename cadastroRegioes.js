...
const atualizar = () => {
    while (true) {
      ...
      listagem();
      const indice = parseInt(prompt("Qual é o índice da região que deseja atualizar? ")) - 1;
      if (indiceInvalido(indice)) {
        console.log("Índice inválido");
      } else {
        const rega = pais[indice];
        console.log("Somente aperte ENTER se não quiser atualizar");
        const novaCidade = prompt("Qual é o novo nome da cidade? (atual: "+rega.cidade+") ");
        if (!nomeInvalido(novaCidade)) {
          rega.cidade = novaCidade;
        }
        const novoEstado = prompt("Qual é o novo nome do estado? (atual: "+rega.estado+") ");
        if (!nomeInvalido(novoEstado)) {
          rega.estado = novoEstado;
        }
        const novaRegiao = prompt("Qual é a nova região do estado? (atual: "+rega.regiao+") ");
        if (!nomeInvalido(novaRegiao)) {
          rega.regiao = novaRegiao;
        }
        const novoPais = prompt("Qual é o novo nome do país? (atual: "+rega.pais+") ");
